...
(define (racketlist->mupllist rl) ; rl is racket list of values
  (if (null? rl)
    (aunit)
  ...
  (cond
    ...
    [(ifgreater? e) (let ([v1 (eval-under-env (ifgreater-e1 e) env)]
                      ...
                      (if (and (int? v1) (int? v2))
                        ...
                        (error "MUPL ifgreater compares two non-numbers")))]
    ...
    [(call? e) (let ([cl (eval-under-env (call-funexp e) env)])
                 (if (closure? cl)
                   (let* ([arg (eval-under-env (call-actual e) env)]
                          [function (closure-fun cl)]
                          [fname (fun-nameopt function)]
                          [argname (fun-formal function)])
                     (eval-under-env
                       (fun-body function)
                       (let ([clenv (cons (cons argname arg) (closure-env cl))])
                         (if fname
                           (cons (cons fname cl) clenv)
                           clenv))))
