...
#   * intersectPoint, intersectLine, and intersectVerticalLine
...
class Point < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    def inbetween(v, end1, end2)
      epsil =  GeometryExpression::Epsilon
      (end1 - epsil <= v && v <= end2 + epsil) || (end2 - epsil <= v && v <= end1 + epsil)
    end
...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    if real_close(seg.x1,seg.x2) #the segments are on a vertical line
      if @y1 <= seg.y1 #let 'self' start at or below of 'seg'
        ...
        seg.intersectWithSegmentAsLineResult self #swap segments
      end
    else
      if @x1 <= seg.x1 #let 'self' start at or to the left of start of 'seg'
...
class Intersect < GeometryExpression
  ...
  def eval_prog env
    @e1.eval_prog(env).intersect @e2.eval_prog(env) #makes intersect-x methods in non-value classes unnecessary, as intersect will always be called with values
  end
  def preprocess_prog
