...
class LineSegment < GeometryValue
  ...
  def LineSegment.Copy(seg)
    new(seg.x1, seg.y1, seg.x2, seg.y2)
  end

  def preprocess_prog
    if real_close_point(x1, y1, x2, y2)
      Point.new(x1, y1)
    elsif (x2 - GeometryExpression::Epsilon < x1)
      LineSegment.new(x2, y2, x1, y1)
    elsif (real_close(x1, x2) and y2 - GeometryExpression::Epsilon < y2)
      LineSegment.new(x2, y2, x1, y1)
    else
      LineSegment.Copy(self)
    end
  end
  ...
  def intersectWithSegmentAsLineResult seg
    if real_close(@x1, @x2)
	  ...
	  if real_close(seg_a.y2, seg_b.y1)
	    ...
	    LineSegment.new(seg_b.x1, seg_b.y1, seg_b.x2, seg_b.y2) # b inside a
      else
        LineSegment.new(seg_b.x1, seg_b.y1, seg_a.x2, seg_a.y2)	# overlapping
      end
    else
