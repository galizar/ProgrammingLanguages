...
class GeometryValue
  ...
  public
  def self.in_between v,end1,end2
    e = GeometryExpression::Epsilon
    (end1 - e <= v and v <= end2 + e) or
    (end2 - e <= v and v <= end1 + e)
  end
...
class Point < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    if GeometryValue::in_between(x, seg.x1, seg.x2)
      Point.new(x, y)
    else NoPoints.new
    end
  end
...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    if real_close_point(x1, y1, x2, y2)
      ...
      if x1 > x2 or (real_close(x1, x2) and y1 > y2)
  ...
  def intersectWithSegmentAsLineResult seg
    x1_real_close_x2 = real_close(x1, x2)
    if x1_real_close_x2
    ...
    thisFields = [x1, y1, x2, y2]
    thatFields = [seg.x1, seg.y1, seg.x2, seg.y2]
    aXstart,aYstart,aXend,aYend = if condition then thisFields else thatFields end
    bXstart,bYstart,bXend,bYend = if condition then thatFields else thisFields end
