         ...
         (case intersect(two_points_to_line seg, v2) of
             ...
             | Point(x0,y0) =>
                 ...
                 let
                   ...
                   val (x1,y1,x2,y2) = seg
                 in
                   if inbetween(x0,x1,x2) (* andalso inbetween(y0,y1,y2) *) (* no need to check if
                   the point is in the range of the segment. the point is the result of an
                   intersection with the line that contains the segment*)
                   then Point(x0,y0)
                   else NoPoints
                 end
  ...
  case e of
     ...
     | Var s =>
         (case List.find (fn (s2,v) => s=s2) env of
             ...
             | SOME (_,v) => v)
     | Shift (deltaX, deltaY, subE) =>
         case eval_prog (subE, env) of
              NoPoints => NoPoints
            | Point (x, y) => Point (x + deltaX, y + deltaY)
            | Line (a, b) => Line (a, (b + deltaY) - (a * deltaX))
            | VerticalLine x => VerticalLine (x + deltaX)
            | LineSegment (x1, y1, x2, y2) =>
                LineSegment (x1 + deltaX, y1 + deltaY, x2 + deltaX, y2 + deltaY)
            | resulting_exp => eval_prog (resulting_exp, env)
  ...
  case exp of
       LineSegment (x1, y1, x2, y2) =>
         ...
         then Point (x1, y1)
           ...
           if (real_close (x1, x2) andalso y1 > y2) orelse x1 > x2
